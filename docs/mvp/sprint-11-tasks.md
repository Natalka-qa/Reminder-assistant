...
- [ ] `npm run build`, `npm run lint`, `npm run typecheck`, `npm run test`, `npm run format:check` зелёные; CI на PR проходит. (2026-09-25 локально — все пять зелёные, 190 тестов, `npm run build` против ветки `dev`, миграций нет; CI — на PR.)
...
- [ ] OAuth-приложение в Google Cloud — **In production** (S11-12) до того, как `GOOGLE_CALENDAR_ENABLED=true` появится в Vercel; до этого на проде флаг не включается. (Шаг 6, после мержа.)
...
**Acceptance criteria**
- [ ] Все сценарии выше пройдены, результат записан в описание PR.
...
**Acceptance criteria**
- [ ] `/privacy` и `/terms` открываются на проде без входа. (2026-09-25 локально: статические страницы вне `(app)`, открываются без входа, ссылки в футере `/` и `/login`; контакт — techremindy@gmail.com; тексты — на ревью. На проде — после мержа.)
- [ ] Publishing status — **In production**; аккаунт не из Test users подключает календарь без «Access blocked» (scope non-sensitive — экрана «unverified app» тоже нет).
- [ ] Флаг на проде включён только после этого.
...
|---|---|---|
| Режим Testing в Google Cloud | Refresh token истекает через 7 дней → «не подключено» раз в неделю; пользователи не из Test users получают «Access blocked» | Publishing status «In production» («Расхождения» п.8) — требует страниц Privacy Policy и Terms of Service (2026-09-25); до этого флаг на проде не включать; README предупреждает о Testing |
...
| 6 | S11-12: после мержа — Branding, Publish app, флаг в Vercel | флаг на проде включён |
